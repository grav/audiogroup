...
<math>\mathit{MSE} = \frac{1}{mn\cdot3}\sum_{i=0}^{m-1}\sum_{j=0}^{n-1} (||I_{r}(i,j) - K_{r}(i,j)||^2 + ||I_{g}(i,j) - K_{g}(i,j)||^2 + ||I_{b}(i,j) - K_{b}(i,j)||^2)</math>,

hvor I_r er R-kanalen i det oprindelige billede, K_r er R-kanalen i det komprimerede billede osv.

Herefter beregnes PSNR:

<math>\mathit{PSNR} = 10 \cdot \log_{10} \left( \frac{\mathit{MAX}_I^2}{\mathit{MSE}} \right)</math>,
